...
export default function Note({
  ...
  const [size, setSize] = useState(
    note.size || { width: 240, height: 180 }
  );
  const [position, setPosition] = useState(
    note.position || {
      x: (index % 4) * 280 + 20, // small left padding
      y: Math.floor(index / 4) * 200 + 20, // small top padding
    }
  );
  ...
  return (
    <Rnd
      bounds={containerRef || "window"} // prevent overlapping navbar
      size={size}
      position={position}
      minWidth={220}
      minHeight={150}
